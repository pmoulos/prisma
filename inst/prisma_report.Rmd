...
```{r selection_basic, fig.width=14, fig.height=6, eval=FALSE}
...
```{r cv_basic_pct, fig.width=14, fig.height=8, eval=FALSE}
if (!is.null(cvMetricsOut)) {
    ...
    for (m in names(prismaOut$results)) {
        if (!is.null(cvMetricsOut[[m]])) {
            ...
            cat(
                "<div>Each of the following tabs contains evaluation plots produced",
...
```{r cv_basic_prs, eval=FALSE}
if (!is.null(cvMetricsOut)) {
    pandoc.header("Cross validation - PRS {.tabset .tabset-pills}",level=1)
...
for (m in names(prismaOut$results)) {
    items <- paste("<option value=",
        ...
        sep="")
    #cat("<div class=\"prs_selector_container\" style='display:none'>")
...
cat("The following tables present the respective PRS cadidates in summary. Use",
    "the selectors above to inspect the tables and the links below to download",
    "the respective PRS candidate in text-tab delimited format (can be opened",
    "in a spreadsheet like Excel.")

# The tables?
for (m in names(prismaOut$results)) {
    for (n in names(prismaOut$results[[m]]$candidates)) {
        #pandoc.header(paste0(n," SNPs"),level=3)
        ...
        lkp <- lookupOut[[m]][[n]]
        if (nrow(lkp) > 0) {
            lkp[,"variant_id"] <- paste("<a href='https://www.ncbi.nlm.nih.gov",
                "/snp/",lkp[,"variant_id"],"' target='_blank'>",
                lkp[,"variant_id"],"</a>",sep="")
            lkp[,"trait"] <- paste("<a href='http://www.ebi.ac.uk/efo/",
                lkp[,"efo_id"],"' target='_blank'>",lkp[,"trait"],"</a>",sep="")
        }
        lkp <- lkp[,-5]
        
        cat("<div class='prstable-helper-wrapper' id='prstable_",m,"_",n,"'>",
            sep="")
        cat("<h4>PRS with ",n," SNPs derived with ",m," effect estimations",
            "</h4>",sep="")
